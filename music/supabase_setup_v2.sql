...
returns uuid language plpgsql security definer set search_path=public as $$declare rid uuid;n integer;begin select count(*)into n from dmq_players where room_id=p_room_id;if not exists(select 1 from dmq_rooms where id=p_room_id and host_user_id=auth.uid() and status='lobby')then raise exception 'Alleen de organisator kan starten.';end if;if n<2 or n>5 then raise exception 'Er moeten 2 tot 5 spelers zijn.';end if;update dmq_players p set score=0,power_used=false,power_cards=(select array_agg(val) from (select unnest(ARRAY['hyperdrive','wild_ride','ghost_whisper','hidden_treasure','second_drop','lightspeed','small_world','ingredient_theft','laser_block']) as val where p.id is not null order by random() limit 3) x),used_cards=array[]::text[] where room_id=p_room_id;update dmq_rooms set status='playing',phase='claim',total_rounds=p_total_rounds,current_round_no=1,game_mode=p_game_mode,song_sequence=p_song_sequence,question_sequence=p_question_sequence,settings=p_settings,updated_at=now() where id=p_room_id;insert into dmq_rounds(room_id,round_no,song_number,question_type,phase)values(p_room_id,1,p_song_sequence[1],p_question_sequence[1],'claim')returning id into rid;return rid;end$$;
...
create or replace function public.dmq_activate_power(p_round_id uuid,p_power text)returns boolean language plpgsql security definer set search_path=public as $$declare r dmq_rounds%rowtype;p dmq_players%rowtype;begin select * into r from dmq_rounds where id=p_round_id for update;select * into p from dmq_players where room_id=r.room_id and user_id=auth.uid() for update;if not found then raise exception 'Je bent geen speler.';end if;if not (p_power = any(p.power_cards)) then raise exception 'Je bezit deze kaart niet.';end if;if p_power = any(p.used_cards) then raise exception 'Je hebt deze kaart al gebruikt.';end if;if p_power = 'laser_block' then if r.active_power is null or r.active_power = 'laser_blocked' then raise exception 'Er is geen actieve kracht om te blokkeren.';end if;update dmq_rounds set active_power='laser_blocked',power_used_by_player_id=p.id where id=p_round_id;else if r.active_power is not null then raise exception 'Er is al een kracht actief.';end if;update dmq_rounds set active_power=p_power,power_used_by_player_id=p.id,power_started_at=now()where id=p_round_id;end if;update dmq_players set used_cards=array_append(used_cards,p_power) where id=p.id;return true;end$$;
...
create or replace function public.dmq_finalize_phantom(p_round_id uuid)returns boolean language plpgsql security definer set search_path=public as $$declare room uuid;begin select room_id into room from dmq_rounds where id=p_round_id;if not dmq_is_leader(room) and not exists(select 1 from dmq_rooms where id=room and host_user_id=auth.uid())then return false;end if;update dmq_answers set answer=coalesce(revised_answer,answer)where round_id=p_round_id;return true;end$$;
