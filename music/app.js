...
async function requestMusicProfileAccess(){const name=activeProfileName();const expectedHash=activeMusicProfilePinHash();if(!name||!expectedHash)return true;for(let attempt=0;attempt<3;attempt++){const pin=window.prompt(`Profiel ${name} is gesloten. Voer de toegangscode in:`);if(pin===null)break;if(!/^\d{4,6}$/.test(pin)){window.alert('De toegangscode bestaat uit 4 tot 6 cijfers.');continue}if(await musicPinHash(pin)===expectedHash)return true;window.alert('Deze toegangscode klopt niet.')}localStorage.removeItem('disney_active_profile');localStorage.removeItem('disney_player_name');location.replace(portalUrl());return false}
