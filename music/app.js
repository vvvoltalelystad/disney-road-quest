...
async function fetchRound(){state.round=null;state.answers=[];if(!state.room?.current_round_no)return;let r=await state.sb.from('dmq_rounds').select('*').eq('room_id',state.room.id).eq('round_no',state.room.current_round_no).maybeSingle();if(r.error)throw r.error;state.round=r.data;if(r.data){let a=await state.sb.from('dmq_answers').select('*').eq('round_id',r.data.id);if(a.error)throw a.error;state.answers=a.data||[]}}
...
async function releaseSong(){let r=await state.sb.rpc('dmq_release_song_v2',{p_round_id:state.round.id});if(r.error)toast(r.error.message);else schedule()}
...
function form(t,a={}){if(t==='full')return `<div class="field"><label>Film</label><input id="ansFilm" value="${esc(a.film||'')}"></div><div class="field"><label>Titel</label><input id="ansTitle" value="${esc(a.title||'')}"></div><div class="field"><label>Jaar</label><input id="ansYear" type="number" value="${esc(a.year||'')}"></div>`;if(t==='year')return `<div class="field"><label>Jaar</label><input id="ansYear" type="number" value="${esc(a.year||'')}"></div>`;return `<div class="field"><label>Antwoord</label><input id="ansText" value="${esc(a[t]||'')}"></div>`}
