...
async function musicPinHash(pin){const bytes=new TextEncoder().encode(String(pin));const digest=await crypto.subtle.digest('SHA-256',bytes);return[...new Uint8Array(digest)].map(byte=>byte.toString(16).padStart(2,'0')).join('')}
function activeMusicProfilePinHash(){const name=activeProfileName();if(!name)return'';const key=profileStorageKey(name);try{const preferences=JSON.parse(localStorage.getItem('disney_profile_preferences')||'{}');return preferences?.[key]?.pinHash||(key==='dagobert'?DAGOBERT_ACCESS_HASH:'')}catch(e){return key==='dagobert'?DAGOBERT_ACCESS_HASH:''}}
function consumeMusicProfileHandoff(expectedHash){let handoff=null;try{handoff=JSON.parse(sessionStorage.getItem(MUSIC_PROFILE_HANDOFF_KEY)||'null')}catch(e){}sessionStorage.removeItem(MUSIC_PROFILE_HANDOFF_KEY);return!!handoff&&handoff.profileKey===profileStorageKey(activeProfileName())&&handoff.pinHash===expectedHash&&Date.now()-Number(handoff.issuedAt||0)<60000}
async function requestMusicProfileAccess(){const name=activeProfileName();const expectedHash=activeMusicProfilePinHash();if(!name||!expectedHash||consumeMusicProfileHandoff(expectedHash))return true;for(let attempt=0;attempt<3;attempt++){const entered=window.prompt(`Profiel ${name} is gesloten. Voer de toegangscode in:`);if(entered===null)break;const pin=String(entered).trim();if(!/^\d{4,6}$/.test(pin)){window.alert('De toegangscode bestaat uit 4 tot 6 cijfers.');continue}if(await musicPinHash(pin)===expectedHash)return true;window.alert('Deze toegangscode klopt niet.')}localStorage.removeItem('disney_active_profile');localStorage.removeItem('disney_player_name');location.replace(portalUrl());return false}
...
async function refreshMusicProfileStore(){if(!state.sb)return;try{const result=await state.sb.from('rooms').select('updated_at:current_task_state->updated_at,coco_bank:current_task_state->coco_bank,coco_profile_preferences:current_task_state->coco_profile_preferences').eq('code','COCO-PROFILES-V1').maybeSingle();if(result.error)throw result.error;const shared=result.data||{};const updated=String(shared.updated_at||'');if(state.profileUpdatedAt&&updated&&updated<=state.profileUpdatedAt)return;if(updated)state.profileUpdatedAt=updated;if(shared.coco_profile_preferences!==undefined)localStorage.setItem('disney_profile_preferences',JSON.stringify(shared.coco_profile_preferences));if(shared.coco_bank!==undefined)localStorage.setItem('disney_coco_coin_bank',JSON.stringify(shared.coco_bank));renderGlobalMusicHeader()}catch(e){console.warn('Disney-profielen konden nog niet worden ververst.',e)}}
function refreshMusicProfileWhenActive(){if(document.visibilityState==='visible')refreshMusicProfileStore()}
function startMusicProfileSync(){if(state.profilePoll)clearInterval(state.profilePoll);if(state.profileChannel&&state.sb)state.sb.removeChannel(state.profileChannel).catch(()=>{});state.profileChannel=state.sb.channel('coco-profiles-music').on('postgres_changes',{event:'UPDATE',schema:'public',table:'rooms',filter:'code=eq.COCO-PROFILES-V1'},refreshMusicProfileStore).subscribe();state.profilePoll=setInterval(refreshMusicProfileStore,300000);window.addEventListener('focus',refreshMusicProfileWhenActive);window.addEventListener('online',refreshMusicProfileStore);document.addEventListener('visibilitychange',refreshMusicProfileWhenActive)}
...
function cleanupAll(){cleanup();if(state.profilePoll)clearInterval(state.profilePoll);state.profilePoll=null;if(state.profileChannel&&state.sb)state.sb.removeChannel(state.profileChannel).catch(()=>{});state.profileChannel=null;window.removeEventListener('focus',refreshMusicProfileWhenActive);window.removeEventListener('online',refreshMusicProfileStore);document.removeEventListener('visibilitychange',refreshMusicProfileWhenActive)}
function render(){
...
async function musicUsedSongNumbers(){
  try{
    const result=await state.sb.from('rooms').select('coco_content_usage:current_task_state->coco_content_usage').eq('code','COCO-PROFILES-V1').maybeSingle();
    if(result.error)throw result.error;
    const usage=result.data?.coco_content_usage||{};
