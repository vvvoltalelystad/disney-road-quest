...
function renderGlobalMusicHeader(){const header=document.getElementById('globalMusicHeader');if(!header)return;const profile=readMusicProfile();if(!profile){header.hidden=true;return}const avatarId=profile.preference?.avatar||'';const avatarFile=avatarId?`avatars/${avatarId}.${avatarId==='linguini'?'webp':'png'}`:'';const color=C(profile.preference?.color).hex;header.hidden=false;header.innerHTML=`<div class="global-profile-pill"><a class="global-profile-segment global-profile-avatar-segment" href="${portalUrl('profile')}" aria-label="Wijzig avatar en spelerskleur">${avatarFile?`<img class="global-profile-avatar" src="${avatarFile}" alt="" style="border-color:${color}">`:`<span class="global-profile-avatar-fallback" style="border-color:${color}">${esc(profile.name.slice(0,1).toUpperCase())}</span>`}</a><a class="global-profile-segment global-profile-name-segment" href="${portalUrl('log')}" aria-label="Open spelersoverzicht van ${esc(profile.name)}"><span>${esc(profile.name)}</span></a><a class="global-profile-segment global-profile-balance-segment" href="${portalUrl('coin')}" aria-label="Bekijk Coco Coin. Saldo: ${profile.balance}"><span>${profile.balance}</span><img src="../collectables/coco-coin-front.png" alt="Coco Coin"></a></div><button class="global-header-icon" type="button" onclick="toggleMusicTheme()" aria-label="Wissel dag- en nachtstand">${musicTheme()==='night'?'☀️':'🌙'}</button><button class="global-header-icon" type="button" onclick="returnToPortal()" aria-label="Terug">←</button>`;applyMusicTheme()}
