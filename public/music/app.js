...
function awardDisneyStars(name,points){const stars=Math.max(0,+points||0);if(stars<=0)return;try{const key=disneyStarKey(name);const bank=JSON.parse(localStorage.getItem('disney_star_bank')||'{}');bank[key]=(bank[key]||0)+stars;localStorage.setItem('disney_star_bank',JSON.stringify(bank));localStorage.setItem('disney_player_name',name||'Speler')}catch(e){}}
